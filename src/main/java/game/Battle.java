...
public class Battle {
    ...
    /*
    public boolean startBossEncounter(Player pc, Boss boss) {
        switch(boss.getShortName()) {
            case "Iri":
                System.out.println( "You enter a brightly lit hall. A long dining table stretches for the full length of the room.\n" +
                                    "At the other end, you see a creature with big, dark wings.\n");
                System.out.println( "\"Impressive. You have made it this far. It's been a long while since I could bathe my blade in blood.\n" +
        ...
        while(pc.getCurrenthealth() != 0 && boss.getCurrenthealth() != 0) {
            int index = 0;
            ...
            for (Character attacker : queue) {
                ...
                if(attacker.getType() == Character.CharacterType.PLAYER) {
                    ...
                    switch(menuChoice) {
                        case "1":
                            int netDamage = calculateCriticalHitChance(attacker);
                            netDamage = calculateNetDamage(attacker, boss, netDamage);
                    }
                }
                else {
                    switch(attacker.getShortName()) {
                        case "Iri":
                            nephilimCombatLogic(attacker, pc);
                            break;
                        case "Ret":
                            underlordCombatLogic(attacker, pc);
                            break;
                        case "God":
                            godCombatLogic(attacker, pc);
                            break;
                    }
                }
            }
    ...
    private void nephilimCombatLogic(Character boss, Player player){
        ...
        if(abilityChance < 0.80) {
            bossInflictDamage(boss, player);
        }
        else {
            bossUseAbility(boss, player);
        }
    }
    ...
    private void bossInflictDamage(Character boss, Player player) {
        int netDamage = calculateCriticalHitChance(boss);
        netDamage = calculateNetDamage(boss, player, netDamage);
        int totalDMG = netDamage;

        switch(((Boss)boss).getDamageType()) {
            case PHYSICAL:
                netDamage = attackArmor(boss, player, netDamage);
                break;
            case MAGICAL:
                netDamage = attackShield(boss, player, netDamage);
                break;
            case UNIVERSAL:
                netDamage = attackArmor(boss, player, netDamage);
                netDamage = attackShield(boss, player, netDamage);
        ...
        if(netDamage > 0) {
            attackHealth(boss, player, netDamage, totalDMG, false, null, null);
        }
    }

    private void bossUseAbility(Character boss, Player player) {
    ...
    private void attackHealth(Character attacker, Character target, int damage, int totalDMG, boolean cleave, ArrayList<Character> foes, ArrayList<Item> roomLoot) {
        if (damage > 0) {
            if (target.getCurrenthealth() <= damage) {
                ...
                else if(foes != null && roomLoot != null){
                    target.updateHealth(-target.getCurrenthealth());
    ...
    private void inflictDamage(Character attacker, Character target, ArrayList<Character> foes, int netDamage,ArrayList<Item> roomLoot, boolean cleave) {
        ...
        switch (damageType) {
            ...
            case PHYSICAL:
                ...
                netDamage = attackArmor(attacker, target, netDamage);
            ...
            case MAGICAL:
                ...
                netDamage = attackShield(attacker, target, netDamage);
        ...
        attackHealth(attacker, target, netDamage, totalDMG, cleave, foes, roomLoot);
